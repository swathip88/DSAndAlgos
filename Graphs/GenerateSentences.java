class GenerateSentences {
    //https://leetcode.com/problems/synonymous-sentences/submissions/
    public List<String> generateSentences(List<List<String>> synonyms, String text) {
        List<String> ans = new ArrayList<>();
        String[] words = text.split(" ");
        Map<Integer, Set<String>> map = new HashMap<>();
        Map<String, String> parent = new HashMap<>();
        for (List<String> syn : synonyms){ // union find
            union(syn.get(0), syn.get(1), parent);
        }

        List<String> syn = new ArrayList<>(parent.keySet());
        for (int i = 0; i < words.length; i++){ // get all possibilities for a given word at index i
            map.computeIfAbsent(i, o -> new HashSet<>()).add(words[i]);
            for (String s : syn){ // we can't use parent.keySet() here (concurrent modification error)
        ...
        dfs(0, map, words, new StringBuilder(), ans); //backtrack
